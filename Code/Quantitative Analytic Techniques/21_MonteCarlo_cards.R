...
# Suppose we have two sets of 10 colored cards.
# The first set of cards has 6 red cards and 4 blue cards.
# The second set of cards has 8 red cards and 2 blue cards. 

# Without looking at the color, we draw one card from the first set and place it in the second set.
# (So the first set now has 9 cards and the second has 11 cards)

# Then -- again without seeing the color -- we draw one card from the second set and place it in 
# the first set.

# If we were to then take one card from the first set, what is the probability that it's a red card?
...
#   We'll do this by picking an integer j between 1 and 10 at random and then taking the jth 
#   card from set_1 and putting it in set_2
...
# Run it again but this time just get the proportion results that are "RED" instead of 
# uselessly printing the all of the outcomes
mean( replicate(10^4, cardsExperiment() == "RED") )

# Let's look at how the estimated probabilities converge as the number of replications gets larger
...
# We can also modify our function to allow the user to pick the size of the decks and the 
# number of red/blue cards in each. 
cardsExperiment <- function(n, r1, r2){ # n = number of cards per set, 
                                        # r1 & r2 = number of reds in set_1 & set_2, respectively 
  
  stopifnot(n >= r1, n >= r2) # give error message if r1 or r2 is greater than the number of cards in set
...
# Use the new function to estimate the desired probability if the decks 
# start with 20 cards and 5 and 16 red cards, respectively
