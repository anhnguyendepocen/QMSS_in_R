...
# But what if we don't want to type or cut/paste all of that everytime we want
# to add a plot for a different age range?

# To make things a bit easier to read and save a lot of space in the code we can
# first make our plotting command into a new function that takes a min and max
# (defining a range of ages) as its argument
...
# Here's an example of stacking 3 histograms (each with a different age range)
# and saving the combined plot First we specify the type of file and the name we
# want to use. We'll use pdf, so we call the pdf function which initiates the
# graphics device driver for producing pdfs (see ?pdf for customizable options)
...
# Suppose instead of stacking the 3 plots we wanted a different file for each of
# them. We bring the pdf creation inside our loop
for(i in 1:3){  
  ...
  dev.off() # also need dev.off() inside the loop so pdf() command starts a new plot each time 
}
...
# Suppose we don't want to save the plots directly to the working directory but
# rather to a folder called "Plots" within the working directory. We can
# manually create the folder and then just change the file name we give to pdf()
# to "Plots/interaction_plot.pdf", or we can have R create the "Plots" folder
# for us. This can be very useful when writing code that generates all sorts of
# different plots or tables that we want to be saved in different directories or
# subdirectories.
...
# If we want, we can now create subdirectories inside of "Plots". Maybe we want
# to save each plot in three file types (png, svg, pdf) so we can choose which 
# to use later. We can create the subdirectories "PDF", "SVG" and "PNG" to
# organize them
...
# HOWEVER, if we come back and run the entire code again later R will try to
# create the directories again and we will get errors because they already
# exist. There are a few ways to avoid this. We could add the argument
# "showWarnings = FALSE" to dir.create() in the loop above, but this could 
# suppress other types of warnings that might be important. Instead we can just
# write a better loop that checks if the directories exist and only creates them
# if they don't
...
# Suppose we want to create histograms for many more age ranges AND we want to
# create pdf, png, and svg versions of each plot and save them in the
# appropriate subdirectory in "Plots"
...
for(i in 1:N){ # loop over the indices of the min, max and colors vectors
  for(TYPE in file.types){ # loop over the file types
    ...
    do.call(TYPE, args = list(file = plot.name)) # do.call() allows us to execute a function from it's name as a character string. the functions we want are already character strings in the file.types vector
    plotByAge(min[i], max[i], border = "white", col = colors[i]) # use our plotByAge function to make the plot
    dev.off()
  }
}  

# After running this loop the should be jpeg, pdf, png plots for each of the age
# ranges in the in the appropriate subdirectory of Plots
