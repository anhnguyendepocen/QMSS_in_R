...
g_age.cohort <- ggplot(age.cohort, aes(x=age.cut, y=NATCRIME, group=cohort.cut)) + ylim(2.3,2.9)
g_age.cohort <- g_age.cohort + geom_point(aes(color = factor(cohort.cut)), size = 3) + geom_line(aes(color=factor(cohort.cut))) 
g_age.cohort
...
g_year.cohort <- ggplot(year.cohort, aes(x=year.cut, y=NATCRIME, group=cohort.cut)) + ylim(2.3,2.9)
g_year.cohort <- g_year.cohort + geom_point(aes(color = factor(cohort.cut)), size = 3) + geom_line(aes(color=factor(cohort.cut)))
g_year.cohort
...
g_age.year <- ggplot(age.year, aes(x=age.cut, y=NATCRIME, group=year.cut)) + ylim(2.3,2.9)
g_age.year <- g_age.year + geom_point(aes(color = factor(year.cut)), size = 3) + geom_line(aes(color=factor(year.cut)))
g_age.year
...
g_year <- ggplot(by.year, aes(x = year, y = NATCRIME)) + ylim(2.3, 2.9)
